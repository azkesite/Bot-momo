...
- `memory-bank/design-document.md`: product requirements, behavior rules, and MVP scope
- `memory-bank/tech-stack.md`: recommended stack and target architecture
- `memory-bank/implementation-plan.md`: step-by-step implementation instructions
- `memory-bank/architecture.md`: frozen execution decisions and architecture constraints

Implementation should follow the structure proposed in `memory-bank/tech-stack.md`:

- `apps/bot-server/`: Fastify entrypoint and platform/webhook handlers
- `packages/core/`: shared types and utilities
- `packages/decision-engine/`: reply gating, active-reply decisions, keyword rules
...
No runnable app is checked in yet. When implementation starts, use the stack defined in `memory-bank/tech-stack.md`:
...
## Testing Guidelines
Use Vitest for unit and integration tests. Place tests next to source files as `*.test.ts` or in package-level `tests/` folders. Prioritize coverage for reply decisions, duplicate-prevention logic, keyword matching, memory writes, and sentence splitting. Run `pnpm test` before opening a PR.

## Commit & Pull Request Guidelines
Local Git history is not available in this workspace, so follow a strict conventional style such as `feat: add active reply decision logic` or `fix: prevent duplicate delayed sends`. Keep commits focused.
